...
class ServiceClientGenerator {
	...
	constructor(options) {
		...
		this.className = replaceAll(Utils.toTitleCase(this.serviceName), " ", "") + "Client";
...
module.exports = ${this.className};`;
	...
	_getEndpointConstant(endpoint) {
		const constantsServiceName = replaceAll(this.serviceName, "-", "_").toUpperCase();

		let constantName = replaceAll(endpoint.subject.toUpperCase(), "-", "_");

		constantName = replaceAll(constantName, ".", "_");
		constantName = constantName.replace(`${constantsServiceName}_`, "");
		constantName = replaceAll(constantName, ":", "");
		constantName = replaceAll(constantName, "*", "");
