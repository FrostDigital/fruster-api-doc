import { MAX_DEPTH_FOR_CIRCULAR_JSON_SCHEMAS } from "../constants/constants";
import { v4 as uuid } from "uuid";
import JsonSchemaCruncher from "./JsonSchemaCruncher";
	...
	*/
	static async derefJsonSchema(schemas, serviceName) {
		...
		async function prepareSchema(schema) {
			try {
				...
				if (!schema.sample) {
					try {
						schema.sample = jsf.generate(schema);
					} catch (err) {
	...
	static _decirlularizeObject(schema) {
		...
		function decircularize(value, depth = 0) {
			if (depth > MAX_DEPTH_FOR_CIRCULAR_JSON_SCHEMAS)
					...
					if (object.hasOwnProperty(key)) {
						if (object && typeof object[key] === "object")
							...
							if (typeof object === "object")
								switch (object[key]) {
									...
									case "email":
										...
										break;
									case "password":
										object["faker"] = "internet.password";
										break;
									case "firstName":
										object["faker"] = "name.firstName";
										break;
									case "middleName":
										object["faker"] = "name.firstName";
										break;
									case "lastName":
										object["faker"] = "name.lastName";
										break;
								}
							...
							if (typeof object[key] === "object")
								switch (key) {
									case "uuid":
										object[key]["faker"] = "random.uuid";
										break;
									case "uri":
										object[key]["faker"] = "internet.url";
										break;
									case "email":
...
export function addUnique(object: any, array: any[]) {
...
export function parseSubjectToAPIUrl(subject: string): ParsedSubject {
	let outputURL = subject;

	outputURL = replaceAll(outputURL, ".", "/");
