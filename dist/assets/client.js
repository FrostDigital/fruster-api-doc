...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(function () {
	    var jsonSchemaJson = "#modal #json-schema-json";
	    var sampleJson = "#modal #sample-json";
	    var copyJsonSchemaJson = "#modal #copy-json-schema-json";
	    var copySampleJson = "#modal #copy-sample-json";

	    $(".request-schema").click(clickEvent);
	    $(".response-schema").click(clickEvent);
	    $(copyJsonSchemaJson).click(function () {
	        return $(jsonSchemaJson).toggle();
	    });
	    $(copySampleJson).click(function () {
	        return $(sampleJson).toggle();
	    });
	    ...
	    function clickEvent(event) {
	        var entryClasses = $(event.currentTarget).attr('class').split(/\s+/);
	        var service = entryClasses[1];
	        var schemaId = entryClasses[2];
	        var schema = window._APP_STATE_.schemasPerService[service] ? window._APP_STATE_.schemasPerService[service].find(function (schema) {
	        ...
	        if ((typeof schema === "undefined" ? "undefined" : _typeof(schema)) === "object" && !!schema) {
	            var schemaToJson = Object.assign({}, schema);
	            delete schemaToJson.sample;

	            var jsonSchemaHtml = getJsonHtml(schemaToJson);
	            var sampleHtml = getJsonHtml(schema.sample);

	            var header = "#modal #header";
	            var jsonSchema = "#modal #json-schema";
	            var jsonSample = "#modal #json-sample";
	    ...
	    function getJsonHtml(json) {
	        var formatter = new JSONFormatter.default(json, 1, {
	            animateOpen: false,
	            animateClose: false
	        });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function(modules) {
	    ...
	    module.exports = function(list, options) {
	        if (false) throw new Error("The style-loader cannot be used in a non-browser environment");
